# Among_Us_IRL
An In real life simulation for the game "Among us", done by Node.js server-client relationship.

## How its done
Needs 3 devices to act as servers on local network. 1 is for reactor sabotage handling, 1 for oxygen, and the third for actual server logic.
every player needs to have a mobile device which they need to carry around and complete tasks on


> *Important Note!*
> This server is ment to be hosted on localhost only, I left several serious vulnerabilities there, because im too lazy.
> Note that the client is completely trusted here, so if you want to play this game, just keep in mind that everyone with decent cybersequrity skills can cheat
> (although they will need some external tools which I dont think is obtainable on mobile)
